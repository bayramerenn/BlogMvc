$(document).ready(function () {
                            ...
                            if (commentResult.Data) {
                                ...
                                $.each(commentResult.Data.Comments.$values,
                                    function (index, comment) {
                                        ...
                                        jqueryTableRow.attr('name', `${newComment.CommentId}`);
                                    });
                                dataTable.draw();
                                $('.spinner-border').hide();
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (commentResult.Data) {
                                Swal.fire(
                                    'Silindi!',
                                    `${id} no'lu yorum başarıyla silinmiştir`,
                                    'success'
                                );
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const commentResult = jQuery.parseJSON(data);
                            console.log(commentResult);
                            console.log(commentResult.Data.Comment.CommentId);
                            if (commentResult.Data) {
                                dataTable.row(tableRow).data([
                                    commentResult.Data.Comment.CommentId,
    ...
    function getButtonsForDataTable(comment) {
        if (!comment.IsActive) {
            return `
                                <button class="btn btn-warning btn-sm btn-approve" data-id="${comment.CommentId
                }"><span class="fas fa-thumbs-up"></span></button>
                                <button class="btn btn-info btn-sm btn-detail" data-id="${comment.CommentId
                }"><span class="fas fa-newspaper"></span></button>
                                <button class="btn btn-primary btn-sm mt-1 btn-update" data-id="${comment.CommentId
                }"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm mt-1 btn-delete" data-id="${comment.CommentId
                }"><span class="fas fa-minus-circle"></span></button>
                                            `;
        }
        return `<button class="btn btn-info btn-sm btn-detail" data-id="${comment.CommentId}"><span class="fas fa-newspaper"></span></button>
                                <button class="btn btn-primary btn-sm mt-1 btn-update" data-id="${comment.CommentId}"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm mt-1 btn-delete" data-id="${comment.CommentId}"><span class="fas fa-minus-circle"></span></button>`
    }
});