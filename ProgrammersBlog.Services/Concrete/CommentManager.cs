...
namespace ProgrammersBlog.Services.Concrete
{
    public class CommentManager : ManagerBase, ICommentService
    {
        ...
        {
            var comment = await UnitOfWork.Comments.GetAsync(c => c.CommentId == commentId);
            if (comment != null)
            {
        ...
        {
            var result = await UnitOfWork.Comments.AnyAsync(c => c.CommentId == commentId);
            if (result)
            {
                var comment = await UnitOfWork.Comments.GetAsync(c => c.CommentId == commentId);
        ...
        {
            var article = await UnitOfWork.Articles.GetByIdAsync(commentAddDto.ArticleId);
            if (article == null)
            {
        ...
        {
            var oldComment = await UnitOfWork.Comments.GetAsync(c => c.CommentId == commentUpdateDto.Id);
            ...
            updatedComment.Article = await UnitOfWork.Articles.GetAsync(a => a.ArticleId == commentUpdateDto.ArticleId);
            await UnitOfWork.SaveAsync();
        ...
        {
            var comment = await UnitOfWork.Comments.GetAsync(c => c.CommentId == commentId, c => c.Article);
            if (comment != null)
            {
                ...
                comment.Article.CommentCount -= 1;
        ...
        public async Task<IResult> HardDeleteAsync(int commentId)
        {
            ...
            if (comment != null)
            {
                ...
                //article.CommentCount = await UnitOfWork.Comments.CountAsync(c => c.ArticleId == comment.ArticleId && !c.IsDeleted);
                //await UnitOfWork.Articles.UpdateAsync(article);
                await UnitOfWork.SaveAsync();
